...
def list_to_csv(list):
    fd = open("C:/Users/user/Desktop/preprocess/module/temp.csv", 'w')
    for rows in list:
        for col in rows:
            fd.write(str(col) + ",")
        fd.write("\n")
...
def gather_info(temp, status, rssi, tx_sector, rx_sector, msg_size):
    if status == 1 and rssi != 0 and msg_size != 2 and msg_size < 1500 and tx_sector < 21:
        rssi_set.append( - rssi)
        tx_sector_set.append(tx_sector)
        rx_sector_set.append(rx_sector)
        if rx_sector is not 1:
            temp[rx_sector - 1][tx_sector - 1] = - rssi
            # print(temp[rx_sector - 1][tx_sector - 1])
        else:
            list_to_csv(temp)
            # run qlearning.py
            cmd = "bash C:/Users/user/Desktop/preprocess/module/learning.sh " + str(os.path.splitext(sys.argv[1])[0])
            result = os.system(cmd)
            # os.system("clear")
            # print("result of os.system() : " + str(result))
        ...
        if not 'time' in line:
            if isfound and iter != 5:
                ...
                if iter == 5:
                    ...
                    gather_info(temp_set, status, rssi, tx_sector, rx_sector, msg_size)
                    iter = 0
                    temp = ''
                    continue
